package de.munsel.spiderlamp.android;
...
public class AndroidBluetoothAcessor implements BluetoothAcessor {
    ...
    public AndroidBluetoothAcessor(AndroidLauncher parent) {
        activity = parent;
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice
        ...
        mHandler.connect(device.getName());
    ...
    public void init() {
        ...
        if (pairedDevices.size() > 0) {
            for (BluetoothDevice device : pairedDevices) {
                mHandler.getPairedDevice(device.getName(), device.getAddress());
            }
        }
...
/*
        ...
        intent.putExtra(EXTRA_DEVICE_ADDRESS, adress);
    ...
    private void connectionFailed() {
        ...
        mHandler.failedToConnect();

        // Start the service over to restart listening mode
        AndroidBluetoothAcessor.this.start();
    }
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            while (mState != STATE_CONNECTED) {
                ...
                if (socket != null) {
                    synchronized (AndroidBluetoothAcessor.this) {
                        switch (mState) {
                            case STATE_LISTEN:
    ...
    /**
     * Bluetooth discovery Broadcast Receiver
     */
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
                    mHandler.discoveredNewDevice(device.getName(), device.getAddress());
                }
                // When discovery is finished, change the Activity title
            } else if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action)) {
                mHandler.finishedDiscovery();
            }
        }
    };
}
